...
def get_contact_maps(u=None, top=None, traj=None, seq_sep=3):
    '''
    ...
    # We want True where |row - col| >= seq_sep
    indices = np.indices((n, n))
    mask_separation = np.abs(indices[0] - indices[1]) >= seq_sep
...
def upper_int_to_map(int_val, N, Mbits):
    '''
    Converts integer representation of contact map back to NxN bool contact map
    '''
    bit_vec = np.empty(Mbits, dtype=np.int8)
    val = int_val
    for b in range(Mbits):
        bit_vec[Mbits - 1 - b] = val & 1
        val = val >> 1

    C = np.zeros((N, N), dtype=np.int8)
    C[np.triu_indices_from(C, k=1)] = bit_vec
